...
const safeContent = value => escapeHtml(value).replace(/&lt;br\s*\/?&gt;/gi, '<br>').replace(/\r?\n/g, '<br>');
...
function openProject(id) {
  ...
  body.innerHTML = `<div class="modal-top"><span class="tag" style="--accent:${escapeHtml(project.accent)}">${escapeHtml(project.topic)}</span><h2>${escapeHtml(project.title)}</h2><p>2026학년도 2학년 고급지구과학 · 학번 ${escapeHtml(project.studentId)}</p>${project.file ? `<a class="launch" href="viewer.html?file=${encodeURIComponent(project.file)}" target="_blank" rel="noopener noreferrer">안전한 환경에서 시뮬레이션 실행 ↗</a>` : ''}</div><div class="modal-grid"><section><h4>개발 동기 및 목적</h4><p>${safeContent(project.purpose)}</p></section><section><h4>과학적 원리 및 수식</h4><p>${safeContent(project.principle)}</p></section><section><h4>시뮬레이션 사용 방법</h4><p>${safeContent(project.how)}</p></section><section><h4>과학적 한계점</h4><p>${safeContent(project.limit)}</p></section></div>${relatedMarkup(project)}${feedbackMarkup()}`;
  modal.showModal();
  $('#closeBtn')?.focus();
